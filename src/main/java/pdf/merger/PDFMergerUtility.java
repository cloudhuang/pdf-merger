...
public class PDFMergerUtility {
    ...
    public void appendDocument(PDDocument destination, PDDocument source) throws IOException {
        ...
        if (srcOutline == null) {
            srcOutline = new PDDocumentOutline();
            PDPage page = srcCatalog.getPages().get(0);
            PDPageFitWidthDestination dest = new PDPageFitWidthDestination();
            dest.setPage(page);
            PDOutlineItem bookmark = new PDOutlineItem();
            bookmark.setDestination(dest);

            bookmark.setTitle("Page ");
            srcOutline.addLast(bookmark);
        }
     ...
     *                     document.
     */
    public void appendDocument(PDDocument destination, PDDocument source, String bookmark) throws IOException {
        if (source.getDocument().isClosed()) {
            throw new IOException("Error: source PDF is closed.");
        }
        if (destination.getDocument().isClosed()) {
        ...
        if (srcOutline == null) {
            srcOutline = new PDDocumentOutline();
            if (srcCatalog.getPages() != null && srcCatalog.getPages().getCount() > 0) {
                PDPage page = srcCatalog.getPages().get(0);
                PDPageFitWidthDestination dest = new PDPageFitWidthDestination();
                dest.setPage(page);
